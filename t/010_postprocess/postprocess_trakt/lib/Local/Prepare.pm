...
#  my $conf = $self->step->conf;
  ...
  my $hangs_dir = $self->res_dir->child("hangs");
  $hangs_dir->mkpath();

  for my $src_file (path("$FindBin::Bin/010_postprocess/data/hangs")->children)
  {
    $src_file->copy($hangs_dir);
  }

  my $crashes_dir = $self->res_dir->child("crashes");
  $crashes_dir->mkpath();

  for my $src_file (path("$FindBin::Bin/010_postprocess/data/crashes")->children)
  {
    $src_file->copy($crashes_dir);
  }
